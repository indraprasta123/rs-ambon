<?php
// Memulai sesi PHP
@session_start(); // Menonaktifkan pelaporan kesalahan
error_reporting(0);
date_default_timezone_set('Asia/Jakarta'); // Mengatur zona waktu default ke Asia/Jakarta

// Set tanggal dan waktu
$date = DATE('Y-m-d'); // Menyimpan tanggal saat ini dalam format Y-m-d
$day = DATE('d'); // Menyimpan hari saat ini
$day_en = DATE('l'); // Menyimpan nama hari saat ini dalam Bahasa Inggris
$month_en = DATE('F'); // Menyimpan nama bulan saat ini dalam Bahasa Inggris
$month = DATE('m'); // Menyimpan nomor bulan saat ini
$year = DATE('Y'); // Menyimpan tahun saat ini
$time = DATE('H:i:s'); // Menyimpan waktu saat ini dalam format H:i:s
$timeNow = DATE('Y-m-d H:i:s'); // Menyimpan tanggal dan waktu saat ini dalam format Y-m-d

//Mengubah format tanggal 
function ubah_tgl2($tanggal)
{
  $pisah = explode('-', $tanggal); // Memisahkan tanggal berdasarkan tanda "-"
  $larik = array($pisah[2], $pisah[1], $pisah[0]); // Membalikkan urutan tanggal
  $satukan = implode('/', $larik); // Menggabungkan kembali dengan tanda "/"
  return $satukan; // Mengembalikan tanggal yang sudah diubah
}
//Mengembalikan nama hari dalam Bahasa Indonesia.
function hari()
{
  $seminggu = array("Minggu", "Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu"); // Array nama-nama hari
  $hari = date("w"); // Mendapatkan nomor hari dalam seminggu
  $hari_ini = $seminggu[$hari]; // Mengambil nama hari sesuai nomor
  return $hari_ini; // Mengembalikan nama hari
}

//Mengembalikan string berisi hari dalam Bahasa Indonesia dan tanggal dalam format Indonesia
function hari_in()
{
  $today_in = hari() . ", " . tgl_indo(date('Y-m-d')); // Menggabungkan hari dan tanggal dalam format Indonesia
  return $today_in; // Mengembalikan string hasil gabungan
}
// Mengembalikan string berisi hari dalam Bahasa Inggris dan tanggal dalam format Inggris
function hari_en()
{
  $today_en = date('l') . ", " . date('F') . " " . date('d') . ", " . date('Y'); // Menggabungkan hari dan tanggal dalam format Inggris
  return $today_en; // Mengembalikan string hasil gabungan
}
// Variabel untuk menyimpan tanggal, tahun, dan waktu sekarang
$tgl_sekarang = date("Ymd"); // Menyimpan tanggal saat ini dalam format Ymd
$thn_sekarang = date("Y"); // Menyimpan tahun saat ini
$time_sekarang = date("H:i:s"); // Menyimpan waktu saat ini dalam format H:i:s

$tanggal = date("Y-m-d"); // Menyimpan tanggal saat ini dalam format Y-m-d
$tgl_jam = date("Y-m-d H:i:s"); // Menyimpan tanggal dan waktu saat ini dalam format Y-m-d H:i:s

// Mengembalikan waktu dalam format HH:MM
function jam_id($tgl)
{
  $tanggal = substr($tgl, 11, 5); // Mengambil substring dari indeks ke-11 sepanjang 5 karakter
  return $tanggal; // Mengembalikan substring yang diambil
}
// Mengembalikan waktu dalam format HH:MM:SS
function jam_indo($timeNow)
{
  $tgl_jam = substr($timeNow, 11, 8); // Mengambil substring dari indeks ke-11 sepanjang 8 karakter
  return $tgl_jam; // Mengembalikan substring yang diambil
}
...
function tanggal_indo($tanggal, $cetak_hari = false)
{
  ...
  $split = explode('-', $tanggal); // Memisahkan tanggal berdasarkan tanda "-"
  $tgl_indo = $split[2] . ' ' . $bulan[(int) $split[1]] . ' ' . $split[0]; // Mengubah format tanggal ke format Indonesia

  if ($cetak_hari) {
    $num = date('N', strtotime($tanggal)); // Mendapatkan nomor hari dalam seminggu
    return $hari[$num] . ', ' . $tgl_indo; // Mengembalikan hari dan tanggal dalam format Indonesia
  }
  return $tgl_indo;
}


// Mengembalikan hari, tanggal, bulan, tahun, dan jam dalam format Indonesia
function format_hari_tanggal($waktu)
{
  ...
  $hr = date('w', strtotime($waktu)); // Mendapatkan nomor hari dalam seminggu
  $hari = $hari_array[$hr]; // Mengambil nama hari sesuai nomor
  $tanggal = date('j', strtotime($waktu)); // Mendapatkan tanggal
  $bulan_array = array(
    1 => 'Januari',
    2 => 'Februari',
    3 => 'Maret',
  ...
  $bl = date('n', strtotime($waktu)); // Mendapatkan nomor bulan
  $bulan = $bulan_array[$bl]; // Mengambil nama bulan sesuai nomor
  $tahun = date('Y', strtotime($waktu)); // Mendapatkan tahun
  $jam = date('H:i:s', strtotime($waktu)); // Mendapatkan waktu dalam format H:i:s
...
// Mengembalikan nama bulan dalam format Indonesia
function bulan_indo($tgl)
{
  $tanggal = substr($tgl, 8, 2); // Mengambil substring dari indeks ke-8 sepanjang 2 karakter
  $bulan = ambilbulan(substr($tgl, 5, 2)); // Mendapatkan nama bulan dari fungsi ambilbulan
  $tahun = substr($tgl, 0, 4); // Mengambil substring dari indeks ke-0 sepanjang 4 karakter
  return $bulan . ' ' . $tahun; // Mengembalikan nama bulan dan tahun
}
...
// Mengembalikan tanggal dalam format Indonesia (variabel tgl_indo dan tgl_ind adalah sinonim)
function tgl_ind($tgl)
{
  $tanggal = substr($tgl, 8, 2); // Mengambil substring dari indeks ke-8 sepanjang 2 karakter
  $bulan = ambilbulan(substr($tgl, 5, 2)); // Mendapatkan nama bulan dari fungsi ambilbulan
  $tahun = substr($tgl, 0, 4); // Mengambil substring dari indeks ke-0 sepanjang 4 karakter
  return $tanggal . ' ' . $bulan . ' ' . $tahun; // Mengembalikan tanggal dalam format Indonesia
}
...
// Mengembalikan nama bulan dalam format Indonesia dari nomor bulan
...
// Mengembalikan nama bulan dalam format singkat Indonesia dari nomor bulan
...
// Mendapatkan format tertentu dari tanggal
function getformat($tGl)
{
  $pisah = explode(' ', $tGl);
  $aray = array($pisah[0], $pisah[1]);
  $get = implode(' ', $aray);
  return $get;
}
// Mengembalikan format jam dengan pola HH:MM:SS
function jam($time)
{
  $pisah = explode(':', $time);
...
// Mengembalikan waktu dari tanggal dan waktu tertentu dalam format HH:MM:SS
