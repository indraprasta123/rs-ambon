...
switch (@$_GET['action']) {
  ...
  case 'registrasi':
    ...
    if (empty($_POST['born'])) {
      ...
      $born = anti_injection($_POST['born']);
    }

    if (empty($_POST['employess_address'])) {
      $error[] = 'tidak boleh kosong';
    } else {
      $employess_address = anti_injection($_POST['employess_address']);
    }
    ...
    if (empty($error)) {
      ...
      if (($extension = "jpg") && ($extension = "jpeg") && ($extension = "gif")) {
        if ($ukuran_file < 50000000) {
          ...
          $query_u = "SELECT employees.id,employees.employees_code,employees.employees_name,employees.employess_address,employees.born,employees.pangkat,employees.shift_id,shift.shift_id,shift.time_in,shift.time_out FROM employees,shift WHERE employees.shift_id=shift.shift_id AND employees.id='$row_user[id]'";
  ...
  case 'profile':
    ...
    if (empty($error)) {
      $update = "UPDATE employees SET employees_name='$employees_name',
            ...
            born='$born',
            employess_address='$employess_address' WHERE id='$row_user[id]'";
      if ($connection->query($update) === false) {
        die($connection->error . __LINE__);
        echo 'Data tidak berhasil disimpan!';
      } else {
