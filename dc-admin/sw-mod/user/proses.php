<?php
session_start();
// Jika sesi kosong, redirect ke halaman login
if (empty($_SESSION['SESSION_USER']) && empty($_SESSION['SESSION_ID'])) {
  ...
  switch (@$_GET['action']) {
    ...
    case 'update':
      ...
      // Validasi input password, jika ada
      if (empty($_POST['password'])) {
        $password = '';
      } else {
        $password_kirim = mysqli_real_escape_string($connection, $_POST['password']);
      ...
      $headers .= "MIME-Version: 1.0\r\n";
      $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
      // Proses update data jika password tidak diubah
      if ($password == '') {
        if (empty($error)) {
        ...
        // Proses update data jika password diubah
        $update = "UPDATE user SET username='$username',
                    fullname='$fullname',
                    email='$email',
                    password='$password',
