<?php
// Cek apakah koneksi database kosong
if (empty($connection)) {
  // Arahkan ke halaman utama jika tidak ada koneksi
  header('location:../../');
} else {
  // Sertakan file panel jika koneksi ada
  include_once 'sw-mod/sw-panel.php';
  echo '
  <div class="content-wrapper">';
  // Mengatur konten berdasarkan parameter 'op' dari URL
  switch (@$_GET['op']) {
    default:
...
<section class="content">
  <div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
      ...
      // Tampilkan tombol "Tambah Baru" jika level user adalah 1
      if ($level_user == 1) {
        echo '
...
<div class="box-body">
          ...
          <table id="swdatatable" class="table table-bordered">
            ...
            <tbody>';
      // Query untuk mengambil data user dan level
      ...
      if ($result->num_rows > 0) {
        ...
        while ($row_a = $result->fetch_assoc()) {
          ...
          // Tampilkan tombol "Ubah" jika level user adalah 1 atau jika level 2 dan ID login sama
          if ($level_user == 1) {
            echo '
                  <a href="#modalEdit" class="btn btn-warning btn-xs enable-tooltip" title="Edit" data-toggle="modal"'; ?>
            onclick="getElementById('txtid').value='<?PHP echo $row_a['user_id']; ?>';getElementById('txtnama').value='<?PHP echo $row_a['fullname']; ?>';getElementById('txtuser').value='<?PHP echo $row_a['username']; ?>';getElementById('txtemail').value='<?PHP echo $row_a['email']; ?>';getElementById('txtlevel').value='<?PHP echo $row_a['level']; ?>';"><i
              class="fa fa-pencil-square-o"></i>
            Ubah</a><?PHP } else {
            // cek level 2 berdasarkan id login
            if ($user_id == $SESSION_ID) {
              echo '<a href="#modalEdit" class="btn btn-warning btn-xs enable-tooltip" title="Edit" data-toggle="modal"'; ?>
              onclick="getElementById('txtid').value='<?PHP echo $row_a['user_id']; ?>';getElementById('txtnama').value='<?PHP echo $row_a['fullname']; ?>';getElementById('txtuser').value='<?PHP echo $row_a['username']; ?>';getElementById('txtemail').value='<?PHP echo $row_a['email']; ?>';getElementById('txtlevel').value='<?PHP echo $row_a['level']; ?>';"><i
                class="fa fa-pencil-square-o"></i> Ubah</a><?PHP
            } else {
              echo '<button class="btn btn-sm btn-warning access-failed" title="Ubah"><i class="fa fa-pencil-square-o"></i> Ubah</button>';
            }
          }
          // Tampilkan tombol "Hapus" jika level user adalah 1
          if ($level_user == 1) {
            echo '
...
<div class="modal fade" id="modalAdd" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-md">
    <div class="modal-content">
      ...
      // Query untuk mengambil level user
  ...
  <div class="modal-dialog modal-md">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">';
      // Tampilkan dropdown level jika level_user adalah 1
      if ($level_user == 1) {
        echo '
          <label>Level</label>
