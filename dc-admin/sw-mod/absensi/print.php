...
<body>';
    ...
    case 'allexcel':
      ...
      if (isset($_GET['building'])) {
        $building = date($_GET['building']);
      }

      $query = "SELECT employees.id,employees.employees_name,employees.position_id,position.position_name,shift.time_in,shift.time_out,building.name FROM employees,position,shift,building WHERE employees.position_id=position.position_id AND employees.shift_id=shift.shift_id AND building.building_id='$building' ORDER BY employees.id DESC";
      $result = $connection->query($query);
      if ($result->num_rows > 0) {
...
<body>';
    ...
    <section class="container_box">
      <div class="row">';
          ...
          echo '
        <p>Nama   : ' . $row['employees_name'] . '</p>
        <p>Jabatan : ' . $row['position_name'] . '</p>
        <p>Lokasi : ' . $row['name'] . '<br>
      <div class="content_box">
        <table class="customTable">
          <thead>
            <tr>
